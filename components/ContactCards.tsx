...
const PLATFORMS: readonly ContactPlatform[] = [
  ...
  },
  {
    ...
    description: "Threads · short notes & thoughts",
  },
  {
    id: "github",
    label: "GitHub",
    ...
    description: "GitHub · code & projects",
  },
];

const PLATFORM_ICONS: Record<ContactPlatform["id"], JSX.Element> = {
  telegram: (
...
export function ContactCards(): JSX.Element {
  return (
    ...
    >
      <p className="mb-4 font-mono text-[10px] uppercase tracking-[0.24em] text-[color:var(--muted-2)]">
        / reach me
      </p>
      <div className="flex flex-col gap-2.5">
        {PLATFORMS.map((p, i) => (
          <motion.a
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.44 + i * 0.08, ease: EASE }}
            href={p.href}
            target="_blank"
          ...
          >
            {/* accent left strip */}
            <span className="card-accent-strip" aria-hidden />
            {/* hover radial glow */}
            ...
            <span className="flex-1 min-w-0">
              <span className="card-tag">{p.description}</span>
              <span className="card-handle">{p.handle}</span>
            </span>

            {/* arrow */}
            <span className="card-arrow-box" aria-hidden>
